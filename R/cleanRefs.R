...
cleanRefs <- function(files) {
  ...
  for (i in 1:length(files)) {
    ...
    if (class(ref) == "try-error") {
      ...
      ### Replace remaining LaTeX tags

      pos <- grep("family:|given:|dropping-particle:|title:|container-title:|volume:", ref)
      ...
      ### Correct Given / Dropping-Particle
      ...
      if (length(pos1) > 0) {
        ...
        for (j in 1:length(family)) {
          ...
          authors[j] <- gsub("  - family: |    given: |    dropping-particle: ", "", authors[j])
        ...
        for (j in 1:length(aut)) {
          ...
          if (length(grep("-", family)) > 0) {
            ...
            family <- paste0(
              ...
              tolower(substr(family, 2, nchar(family)))
            )
          }


          ### Some correction (O'Neill, McAdams)

          if (length(grep("'", family)) > 0) {
            yyy <- strsplit(family, "'")[[1]]
            family <- yyy[1]
            for (k in 2:length(yyy)) {
              family <- c(family, paste0(toupper(substr(yyy[k], 1, 1)), substr(yyy[k], 2, nchar(yyy[k]))))
            }
            family <- paste0(family, collapse = "'")
          }
          ...
          ### Is there particle (de, van, von, etc.)

          particles <- c(
            "de", "d'", "du", "des", "of",
            "da", "dall'", "degli", "dei", "del", "dell'", "della", "lo", "san",
            "am", "an", "auf", "aus", "der", "im", "von", "und", "zu", "zum", "zur",
            "del", "los", "las",
            "den", "ten", "ter", "te", "van", "vanden", "vander",
            "das", "do", "dos",
            "af", "av"
          )
              ...
              if (length(pos) > 0) {
                ...
                for (z in 1:length(yyy)) {
                  if (!(toupper(substr(yyy[z], 1, 1)) %in% LETTERS)) {
                    if ((toupper(substr(yyy[z], 2, 2)) %in% LETTERS)) {
                      zzz <- c(
                        zzz,
                        toupper(substr(yyy[z], 2, 2)),
                        ".-"
                      )
                    } else {
                      zzz <- c(
                        zzz,
                        toupper(substr(yyy[z], 1, 1)),
                        ".-"
                      )
                    }
                  } else {
                ...
                if (!(toupper(substr(xxx[k], 1, 1)) %in% LETTERS)) {
                  if ((toupper(substr(xxx[k], 2, 2)) %in% LETTERS)) {
                    zzz <- c(
                      toupper(substr(xxx[k], 2, 2)),
                      "."
                    )
                  } else {
                    zzz <- c(
                      toupper(substr(xxx[k], 1, 1)),
                      "."
                    )
                  }
                } else {
