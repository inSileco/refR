      ...
      if (length(grep("^@[[:alpha:]]{1,}\\{", data[i])) > 0) {
        ...
        if (msg == 0) {
          ...
          if (length(data) == 1) {
            ...
            data2 <- yaml.load_file(paste0(path, "/", filename, ".yml"))$references[[1]]

          } else {

              data2[[k + 1]] <- yaml.load_file(paste0(path, "/", filename, ".yml"))$references[[1]]
          }
    ...
    for (i in 1:length(file)) {
      ...
      if (length(which(dir() == paste0(file[i], ".bib"))) == 1) {
        ...
        if (length(grep("^@[[:alpha:]]{1,}\\{", data)) == 1) {
          ...
          system(paste0("pandoc-citeproc --bib2yaml ", file[i], ".bib > ", path, "/", file[i], ".yml"))
          ...
          if (msg == 0) {
            ...
            if (length(file) == 1) {
              ...
              data2 <- yaml.load_file(paste0(path, "/", file[i], ".yml"))$references[[1]]

            } else {

                data2[[k + 1]] <- yaml.load_file(paste0(path, "/", file[i], ".yml"))$references[[1]]
            }
